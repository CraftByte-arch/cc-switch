            ...
            if current_config.enabled {
                ...
                if has_backup && live_matches_current_proxy {
                    if matches!(app, AppType::Codex) {
                        // Keep managed OAuth accounts in sync even when the current
                        // live configuration can be reused without a full takeover.
                        ...
                        // Routed Codex has a generated live config; refresh that
                        // config instead of treating the existing provider config as
                        // the source of truth.
                        if self.codex_model_routing_enabled()? {
